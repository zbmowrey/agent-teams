...
4. Supports the existing status convention (🔴🟡🟢🔵✅⛔) referenced in skill files
...
Each roadmap item gets its own file in `docs/roadmap/` named `{priority}-{slug}.md` (e.g., `P1-01-concurrent-write-safety.md`). This eliminates write conflicts entirely — no two agents should ever modify the same roadmap item simultaneously.
...
| `spec_in_progress`  | 🟡    | Spec being written         |
...
- **Positive**: Human-readable — anyone can browse `docs/roadmap/` and understand the backlog.
